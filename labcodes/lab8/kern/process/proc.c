...
alloc_proc(void) {
  ...
  //LAB8:EXERCISE2 2011011268 HINT:need add some code to init fs in proc_struct, ...
    proc->filesp = NULL;
    }
    return proc;
}
...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
    ...
    //LAB8:EXERCISE2 2011011268  HINT:how to copy the fs in parent's proc_struct?
    /*
...
load_icode(int fd, int argc, char **kargv) {
    /* LAB8:EXERCISE2 2011011268  HINT:how to load the file with handler fd  in to process's memory? how to setup argc/argv?
     * MACROs or Functions:
  ...
  if (current->mm != NULL) 
    ...
    ph = &phdr;
    uint32_t num = 0;
    for (num = 0; num < elf->e_phnum; num++) {
        off_t tempoff = elf->e_phoff + sizeof(struct proghdr) *num;
        if ((ret = load_icode_read(fd, ph, sizeof(struct proghdr), tempoff)) != 0)
        ...
        if (ph->p_flags & ELF_PF_X) 
            temp_flags |= VM_EXEC;        
        uint32_t perm = PTE_U;
        if (temp_flags & VM_WRITE) 
            perm |= PTE_W;        
        if ((ret = mm_map(mm, ph->p_va, ph->p_memsz, temp_flags, NULL)) != 0)
            goto bad_cleanup_mmap;
        ret = -E_NO_MEM;        
        struct Page *page;
        off_t begin = ph->p_offset;
        size_t off, size;
        ...
        while (start < end) {
            ...
            size = PGSIZE - off;
            if (end < la){
                uintptr_t temp = la - end;
                size -= temp;
            }
            if ((ret = load_icode_read(fd, page2kva(page) + off, size, begin)) != 0)
                goto bad_cleanup_mmap;            
            start += size;
            begin += size;
        }       
    ...
    lcr3(PADDR(mm->pgdir));
    uintptr_t t = USTACKTOP - argc*PGSIZE;
    char** uargv = (char **)(t - argc*sizeof(char *));
    int i;
    for (i = 0; i < argc; i++)
        uargv[i] = strcpy((char *)(t + i*PGSIZE), kargv[i]);    
    t = (uintptr_t)uargv - sizeof(int);
    *(int *)t = argc;
